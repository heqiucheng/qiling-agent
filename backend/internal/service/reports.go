...
func renderReportExport(report domain.Report, format string) (ReportExport, error) {
	...
	case "docx":
		body, err := renderReportDOCX(report)
		if err != nil {
			return ReportExport{}, err
		}
		return ReportExport{
			Filename:    safeReportExportFilename(report.ID) + ".docx",
			ContentType: reportDOCXContentType,
			Body:        body,
		}, nil
	case "pdf":
		body, err := renderReportPDF(report)
		if err != nil {
			return ReportExport{}, err
		}
		return ReportExport{
			Filename:    safeReportExportFilename(report.ID) + ".pdf",
			ContentType: reportPDFContentType,
			Body:        body,
		}, nil
	default:
